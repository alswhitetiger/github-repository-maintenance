# GitHub 공개 저장소 상태

점검일: 2026-09-02 (Asia/Seoul)

- 연결 저장소: 42개
- 개인 소유: 33개
- 자동 점검 대상(소유·비포크·비보관): 29개
- 공개 상세 표시: 9개
- 비공개 상세: 로컬에만 20개
- 공동작업 저장소: 읽기 전용 점검 9개

| 저장소 | 기본 브랜치 | 최근 푸시 | 점검 결과 |
|---|---|---:|---|
| [ai-dept-meeting](https://github.com/alswhitetiger/ai-dept-meeting) | `main` | 2026-07-14 | .gitignore 없음; 공개 저장소 라이선스 없음 |
| [AI-Storyteller-Text-Adventure](https://github.com/alswhitetiger/AI-Storyteller-Text-Adventure) | `main` | 2026-06-24 | .gitignore 없음; 공개 저장소 라이선스 없음 |
| [Code-Horizon-team](https://github.com/alswhitetiger/Code-Horizon-team) | `main` | 2026-06-24 | 공개 저장소 라이선스 없음 |
| [find_word_game](https://github.com/alswhitetiger/find_word_game) | `main` | 2026-06-24 | .gitignore 없음; 공개 저장소 라이선스 없음 |
| [github-repository-maintenance](https://github.com/alswhitetiger/github-repository-maintenance) | `main` | 2026-09-02 | 공개 저장소 라이선스 없음 |
| [LLM-game](https://github.com/alswhitetiger/LLM-game) | `main` | 2026-06-24 | .gitignore 없음; 공개 저장소 라이선스 없음 |
| [mingyun-portfolio](https://github.com/alswhitetiger/mingyun-portfolio) | `main` | 2026-07-03 | README 없음; .gitignore 없음; 공개 저장소 라이선스 없음 |
| [My-Emotional-Analysis-Model](https://github.com/alswhitetiger/My-Emotional-Analysis-Model) | `main` | 2026-06-24 | .gitignore 없음; 공개 저장소 라이선스 없음 |
| [Voice-Chat-Bot-Game](https://github.com/alswhitetiger/Voice-Chat-Bot-Game) | `main` | 2026-06-24 | .gitignore 없음; 공개 저장소 라이선스 없음 |

> 이 보고서는 저장소 메타데이터와 루트 파일을 가볍게 확인합니다. 전체 코드 보안 감사나 테스트 실행을 대신하지 않습니다.
